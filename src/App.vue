...
const mostrarSidebar = computed(() => {
  return auth.isAuthenticated &&
         route.path !== '/login' &&
         route.path !== '/register' &&
         route.path !== '/' // Esto asegura que la Landing se vea limpia
})
...
const linkBase = "flex items-center gap-4 px-4 py-4 rounded-[1.5rem] transition-all duration-300 group italic uppercase tracking-[0.15em] text-[10px] font-black no-underline mb-1"
...
const navLabel = "text-[8px] text-slate-500 px-4 mt-4 mb-2 uppercase tracking-[0.2em] font-black italic transition-opacity"
...
function fotoPerfilUrl(path: string | null | undefined): string | null {
  if (!path) return null
  const storageBase = (import.meta.env.VITE_STORAGE_URL as string || 'https://natita.me/storage').replace(/\/$/, '')
  return `${storageBase}/${path.replace(/^\/storage\//, '').replace(/^storage\//, '')}`
}
</script>
...
<template>
  ...
  <div class="flex min-h-screen bg-[#f8fafc] font-sans selection:bg-blue-100 italic">
    ...
    >
      ...
      >
        <svg :class="['w-4 h-4 transition-transform duration-500', isCollapsed ? 'rotate-180' : '']" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-width="3" d="M15 19l-7-7 7-7" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </button>
      ...
      <nav class="flex-1 overflow-y-auto px-4 no-scrollbar">
        ...
        <template v-if="auth.isAdmin">
          ...
          <router-link to="/admin/empleados" :class="[linkBase, route.path.startsWith('/admin/empleados') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8z" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Usuarios</span>
          </router-link>
          <router-link to="/admin/mascotas" :class="[linkBase, route.path.startsWith('/admin/mascotas') ? linkActive : linkInactive]">
          ...
          <router-link to="/admin/servicios" :class="[linkBase, route.path.startsWith('/admin/servicios') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Servicios</span>
          </router-link>

          <p v-if="!isCollapsed" :class="navLabel">Operación</p>
          <router-link to="/admin/citas" :class="[linkBase, route.path.startsWith('/admin/citas') ? linkActive : linkInactive]">
          ...
          <router-link to="/admin/calendario" :class="[linkBase, route.path.startsWith('/admin/calendario') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M8 14h.01M12 14h.01M16 14h.01" stroke-linecap="round" stroke-linejoin="round"/><rect x="3" y="4" width="18" height="18" rx="2"/></svg>
            <span v-if="!isCollapsed">Calendario</span>
          </router-link>
          <router-link to="/admin/walk-in" :class="[linkBase, route.path.startsWith('/admin/walk-in') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Walk-in</span>
          </router-link>

          <p v-if="!isCollapsed" :class="navLabel">Reportes</p>
          <router-link to="/admin/historial" :class="[linkBase, route.path.startsWith('/admin/historial') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M9 12h6M9 16h6M9 8h6M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" stroke-linecap="round" stroke-linejoin="round"/></svg>
        ...
        <template v-if="auth.isRecepcionista">
          <p v-if="!isCollapsed" :class="navLabel">Operación</p>
          <router-link to="/recepcionista/dashboard" :class="[linkBase, route.path.startsWith('/recepcionista/dashboard') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M3 12L12 3l9 9M5 10v9a1 1 0 001 1h4v-5h4v5h4a1 1 0 001-1v-9" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Inicio</span>
          </router-link>
          <router-link to="/recepcionista/citas" :class="[linkBase, route.path.startsWith('/recepcionista/citas') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Agenda</span>
          </router-link>
          <router-link to="/recepcionista/clientes" :class="[linkBase, route.path.startsWith('/recepcionista/clientes') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8z" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Clientes</span>
          </router-link>
          <router-link to="/recepcionista/walk-in" :class="[linkBase, route.path.startsWith('/recepcionista/walk-in') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Walk-in</span>
          </router-link>
        </template>
        ...
        <template v-if="auth.isVeterinario">
          ...
          <router-link to="/veterinario/mascotas" :class="[linkBase, route.path.startsWith('/veterinario/mascotas') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 21c-4 0-7-3-7-7 0-2 1-3.5 3-4.5s4-1 4-1 2 0 4 1 3 2.5 3 4.5c0 4-3 7-7 7z" stroke-linecap="round" stroke-linejoin="round"/><circle cx="9" cy="4" r="1.5" fill="currentColor"/><circle cx="15" cy="4" r="1.5" fill="currentColor"/></svg>
        ...
        <template v-if="auth.isCliente">
          ...
          <router-link to="/client/dashboard" :class="[linkBase, route.path.startsWith('/client/dashboard') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M3 12L12 3l9 9M5 10v9a1 1 0 001 1h4v-5h4v5h4a1 1 0 001-1v-9" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span v-if="!isCollapsed">Inicio</span>
          </router-link>
          <router-link to="/client/citas" :class="[linkBase, route.path.startsWith('/client/citas') ? linkActive : linkInactive]">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18" stroke-linecap="round" stroke-linejoin="round"/></svg>
        ...
        <router-link :to="auth.isCliente ? '/cliente/perfil' : '/perfil'" :class="[linkBase, route.path.includes('perfil') ? linkActive : linkInactive]">
          <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z" stroke-linecap="round" stroke-linejoin="round"/></svg>
          <span v-if="!isCollapsed">Mi Perfil</span>
        </router-link>
      </nav>

      <div class="p-6 border-t border-white/5 bg-slate-950/40 transition-all">
        <div :class="['flex items-center gap-3 transition-all duration-300', isCollapsed ? 'justify-center' : 'mb-6 px-2']">
          <div class="w-10 h-10 rounded-xl bg-[#3f98ff]/20 border border-[#3f98ff]/30 flex items-center justify-center overflow-hidden shrink-0 shadow-lg">
          ...
          <div v-if="!isCollapsed" class="min-w-0">
            <p class="text-white text-[10px] font-black uppercase truncate leading-none italic">{{ auth.user?.name }}</p>
            <p class="text-slate-500 text-[8px] font-bold uppercase tracking-widest mt-1">{{ auth.roleName }}</p>
          </div>
        </div>
