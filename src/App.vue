...
const linkBase =
  'text-white/70 hover:text-white text-[10px] font-black uppercase tracking-[0.15em] px-4 py-2 rounded-xl transition-all no-underline whitespace-nowrap'
...
<template>
  <nav
    class="fixed top-0 left-0 right-0 z-[100] bg-[#3f98ff] h-[64px] px-6 md:px-12 flex items-center justify-between shadow-lg border-b border-white/10 italic"
  >
    ...
    <div class="flex items-center gap-3 shrink-0">
      ...
      <div
        ...
        class="hidden lg:flex flex-col items-end leading-none gap-1 mr-2"
      >
        <span class="text-white font-black text-xs uppercase tracking-tighter">{{
          auth.user?.name
        }}</span>
        <span class="text-blue-200 text-[9px] font-black uppercase tracking-widest">{{
          auth.roleName
        }}</span>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        >
          <div
            class="w-9 h-9 rounded-xl border-2 border-white/20 overflow-hidden group-hover:border-white transition-all shadow-lg bg-blue-400/30 flex items-center justify-center"
          >
            <img
