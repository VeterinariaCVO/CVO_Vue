...
const login = async () => {
  ...
  try {
    // Usamos la función login de tu store (la que ya usa ApiUseFetch)
    await auth.login(form.email, form.password)

    // Si no lanza error, redirigimos según el rol guardado en el store
    ...
    // Capturamos el error que lanza el store (Credenciales incorrectas, etc)
      ...
      <div class="w-full max-w-md bg-white/95 backdrop-blur-2xl rounded-[3.5rem] shadow-2xl p-10 md:p-14 border border-white/40">
        ...
        <form @submit.prevent="login" class="space-y-6">
          ...
          <button
            type="submit"
            :disabled="isLoading"
            class="group relative w-full bg-slate-900 text-white py-5 rounded-2xl overflow-hidden font-black text-xl uppercase italic shadow-xl active:scale-95 transition-all"
          >
        ...
        <div class="mt-12 text-center border-t border-slate-100 pt-8">
          <p class="text-slate-400 text-xs font-bold italic">
            ...
            <button @click="router.push('/register')" class="text-[#3f98ff] font-black hover:underline ml-1 uppercase text-[11px] tracking-widest transition-all">
              Regístrate aquí
            </button>
          </p>
        </div>
