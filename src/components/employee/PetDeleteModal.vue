  ...
  <div v-if="open" class="backdrop" @click.self="$emit('close')">
    <div class="modal">
      ...
      <div class="px-6 py-5 flex flex-col gap-3">
        <p class="text-sm text-gray-700">¿Desea eliminar esta mascota? Esta accion no se puede deshacer.</p>
        <p v-if="hasCitas" class="text-xs text-red-500 bg-red-50 border border-red-200 px-3 py-2 rounded-md">
          Esta mascota tiene citas activas. Se eliminaran junto con la mascota.
        </p>
      </div>
...
defineProps<{ open: boolean; loading: boolean; hasCitas: boolean }>()
...
.modal-header { @apply px-6 py-5 border-b border-gray-100; }
