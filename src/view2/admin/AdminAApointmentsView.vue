...
async function cambiarEstado(citaId: number, accion: string) {
  const { data, statusCode, execute } = ApiUseFetch(`/admin/appointments/${citaId}`)
    .put({ accion })
    .json()
  await execute()
...
async function ejecutarCancelar() {
  ...
  const { data, statusCode, execute } = ApiUseFetch(
    `/admin/appointments/${citaACancelar.value.cita_id}`,
  )
    .delete()
    .json()
  await execute()
      ...
      <table v-else class="w-full border-collapse">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-5 py-3 text-sm text-slate-500">{{ cita.fecha?.slice(0, 10) ?? '—' }}</td>
