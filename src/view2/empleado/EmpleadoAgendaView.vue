...
async function obtenerCitas() {
  ...
  await execute()
  // La API devuelve vw_agenda_empleado (campos planos)
  citas.value = data.value?.data ?? []
  cargando.value = false
}

function abrirReagendar(citaId: number) {
  citaReagendarId.value = citaId
...
async function cambiarEstado(citaId: number, accion: string) {
  // ✅ Usamos accion: 'confirmar' o 'completar' en lugar de status directo
  const { data, statusCode, execute } = ApiUseFetch(`/empleado/appointments/${citaId}`)
    .put({ accion })
    .json()
  await execute()
  ...
  mensajeExito.value = labels[accion] ?? 'Estado actualizado'
  setTimeout(() => (mensajeExito.value = ''), 3000)
  obtenerCitas()
}

function confirmarCancelar(citaId: number) {
  // ✅ Buscamos por cita_id en lugar de id
  const cita = citas.value.find((c: any) => c.cita_id === citaId) ?? null
...
async function ejecutarCancelar() {
  ...
  // ✅ Usamos cita_id en lugar de id
  const { data, statusCode, execute } = ApiUseFetch(
    `/empleado/appointments/${citaACancelar.value.cita_id}`,
  )
    .delete()
    .json()
  await execute()
  ...
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-slate-400 mt-0.5 mb-0">Gestión de citas</p>
      </div>
      <button
      ...
      <table v-else class="w-full border-collapse">
        <thead>
          <tr class="border-b border-slate-100">
            ...
            <th class="text-left text-xs text-slate-400 font-medium px-5 py-3">
              Mascota / Servicio
            </th>
        ...
        <tbody>
          ...
          >
            <!-- ✅ cliente_nombre en lugar de client?.name -->
            <td class="px-5 py-3 text-sm text-slate-800">{{ cita.cliente_nombre }}</td>
            <td class="px-5 py-3">
              <!-- ✅ mascota_nombre y servicio_nombre en lugar de pet?.name y service?.name -->
              <p class="text-sm text-slate-800 m-0">{{ cita.mascota_nombre }}</p>
              <p class="text-xs text-slate-400 m-0">{{ cita.servicio_nombre }}</p>
            </td>
            <!-- ✅ fecha en lugar de time_slot?.date -->
            <td class="px-5 py-3 text-sm text-slate-500">
              {{ cita.fecha?.slice(0, 10) ?? '—' }}
            </td>
            <!-- ✅ start_time en lugar de time_slot?.start_time -->
            <td class="px-5 py-3 text-sm text-slate-500">
            ...
            <td class="px-5 py-3">
              <div class="flex items-center justify-end gap-1">
                ...
                <button
                  ...
                  @click="cambiarEstado(cita.cita_id, 'confirmar')"
                  title="Confirmar"
                ...
                <!-- Completar (confirmed o in_progress) -->
                <button
                  v-if="cita.status === 'confirmed' || cita.status === 'in_progress'"
                  @click="cambiarEstado(cita.cita_id, 'completar')"
                  title="Completar"
                ...
                <button
                  ...
                  @click="abrirReagendar(cita.cita_id)"
                  title="Reagendar"
                ...
                <button
                  ...
                  @click="confirmarCancelar(cita.cita_id)"
                  title="Cancelar"
      ...
      >
        <div class="bg-white rounded-xl border border-slate-200 p-6 max-w-xs w-11/12">
          ...
          <p class="text-sm text-slate-400 m-0 mb-5 leading-relaxed">
            ...
            <!-- ✅ cliente_nombre y mascota_nombre en lugar de client?.name y pet?.name -->
            <span class="text-slate-600">{{ citaACancelar?.cliente_nombre }}</span>
            con <span class="text-slate-600">{{ citaACancelar?.mascota_nombre }}</span
            >.
          </p>
