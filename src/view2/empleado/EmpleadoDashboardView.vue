...
function getHoraClase(cita: any): string {
  // ✅ is_walk_in sigue igual, start_time ahora es directo (no anidado)
  if (cita.is_walk_in || !cita.start_time) {
...
// ✅ Filtramos por fecha usando el campo plano "fecha" de vw_agenda_empleado
const citasHoy = computed(() =>
  citas.value.filter((c: any) => {
    if (c.is_walk_in) return c.created_at?.slice(0, 10) === hoy.value
    return c.fecha === hoy.value
  }),
)
...
const statsData = computed(() => [
  ...
  {
    label: 'En Clínica',
    val: citasHoy.value.filter((c: any) => c.status === 'confirmed' || c.status === 'in_progress')
      .length,
    color: 'bg-emerald-500',
  },
  {
    label: 'Walk-ins Hoy',
    val: citasHoy.value.filter((c: any) => c.is_walk_in && c.status !== 'cancelled').length,
    color: 'bg-purple-500',
  },
  {
    label: 'Pendientes',
    val: citasHoy.value.filter((c: any) => c.status === 'pending').length,
    color: 'bg-amber-500',
  },
])
...
async function cargarDatos() {
  ...
  try {
    ...
    await execute()
    // La API devuelve vw_agenda_empleado con campos planos
  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <main class="p-8 max-w-[1600px] mx-auto space-y-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <section
          class="lg:col-span-3 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden"
        >
          <div
              ...
              <tbody class="divide-y divide-slate-50">
                ...
                >
                  <td class="px-8 py-5 text-xs">
                    <!-- ✅ start_time directo en lugar de time_slot?.start_time -->
                    <span :class="getHoraClase(cita)">
                      {{ cita.is_walk_in ? 'WALK-IN' : (cita.start_time?.slice(0, 5) ?? '—') }}
                    </span>
                  </td>

                  <td class="px-8 py-5">
                    <!-- ✅ mascota_nombre y servicio_nombre en lugar de pet?.name y service?.name -->
                    ...
                    <div class="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                      {{ cita.servicio_nombre ?? 'General' }}
                    </div>
                  </td>

                  <td class="px-8 py-5">
                    <!-- ✅ cliente_nombre y cliente_telefono en lugar de client?.name y client?.phone -->
                  ...
                  <td class="px-8 py-5 text-center">
                    <div class="flex items-center justify-center gap-4">
                      <!-- ✅ mascota_id en lugar de pet?.id -->
                      <button
                        v-if="cita.mascota_id"
                        @click="router.push(`/recepcionista/mascotas/${cita.mascota_id}`)"
                      ...
                      <button
                        v-if="cita.mascota_id"
                        @click="router.push(`/recepcionista/mascotas/${cita.mascota_id}/historial`)"
                      ...
                      <span
                        v-if="!cita.mascota_id"
                        class="text-[9px] font-black text-slate-200 uppercase tracking-widest"
                        >—</span
                      >
                    </div>
                  </td>
        ...
        <aside class="space-y-4">
          <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <p class="text-[9px] text-blue-200 font-bold uppercase tracking-tight">
                  Agendar consulta
                </p>
              </button>
              ...
              >
                <p
                  class="text-xs font-black text-slate-700 uppercase tracking-widest group-hover:text-purple-700"
                >
                  Atención Walk-in
                </p>
                <p
                  class="text-[9px] text-slate-400 font-bold uppercase tracking-tight group-hover:text-purple-400"
                >
                  Sin cita previa
                </p>
              </button>
              ...
              >
                <p
                  class="text-xs font-black text-slate-700 uppercase tracking-widest group-hover:text-[#1d6bbf]"
                >
                  Clientes
                </p>
          ...
          <div
            class="p-6 rounded-3xl border border-[#1d6bbf]/20 bg-gradient-to-br from-[#1d6bbf]/10 via-white to-slate-50 shadow-sm"
          >
            <p class="text-[10px] font-black text-[#1d6bbf] uppercase tracking-widest mb-1">Hoy</p>
            <p class="text-lg font-black text-slate-800 leading-tight">{{ fechaHoyCorta }}</p>
            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-2">
              {{ fechaHoyLarga }}
            </p>
            <p
              class="mt-3 inline-flex items-center rounded-full bg-[#1d6bbf]/10 px-3 py-1 text-[10px] font-black tracking-wider text-[#1d6bbf]"
            >
              {{ horaEnVivo }}
            </p>
          </div>
