...
const deleteModal = ref<{ open: boolean; loading: boolean; pet: Pet | null }>({
  open: false, loading: false, pet: null
})
...
function openEdit(pet: Pet) { petModal.value = { open: true, editPet: pet } }
function openDelete(pet: Pet) { deleteModal.value = { open: true, loading: false, pet } }
...
.client-section { display: flex; align-items: center; gap: 24px; }
.client-avatar {
  width: 64px; height: 64px; border-radius: 50%;
  background: #3f98ff; color: white;
  display: flex; align-items: center; justify-content: center;
  font-size: 26px; font-weight: 700; flex-shrink: 0;
}
.client-name  { font-size: 20px; font-weight: 700; color: #1f2937; }
.client-email { font-size: 13px; color: #6b7280; margin: 4px 0 10px; }
...
.section-title {
  font-size: 18px; font-weight: 700; color: #007bff;
...
.pets-grid { display: flex; flex-direction: column; gap: 12px; }
