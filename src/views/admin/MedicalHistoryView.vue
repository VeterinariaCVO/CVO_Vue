...
async function cargarMascotas() {
  ...
  const url = esCliente.value ? `/pets?owner_id=${auth.user?.id}` : '/admin1/pets'
  ...
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-slate-400 mt-0.5 mb-0">
          Consulta los expedientes médicos de las mascotas
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-5">
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div class="px-4 py-3.5 border-b border-slate-100">
            ...
            <div class="relative">
              ...
              <input
                ...
                class="w-full pl-8 pr-3 py-1.5 text-sm rounded-lg border border-slate-200 outline-none focus:border-[#1d6bbf] transition-colors bg-slate-50"
              />
            </div>
          </div>

          <div v-if="cargandoMascotas" class="text-sm text-slate-400 text-center py-8">
            Cargando...
          </div>
          <div
          ...
          <div v-else class="divide-y divide-slate-50 max-h-[calc(100vh-280px)] overflow-y-auto">
            <button
              ...
              :class="
                mascotaSeleccionada?.id === mascota.id ? 'bg-blue-50' : 'bg-white hover:bg-slate-50'
              "
            >
              <div
                class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center shrink-0 overflow-hidden"
              >
                <img
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-slate-400 m-0 truncate">
                  {{ mascota.species }} · {{ mascota.owner?.name ?? '—' }}
                </p>
              </div>
      ...
      <div class="col-span-2">
        ...
        <div
          v-if="!mascotaSeleccionada"
          class="bg-white rounded-xl border border-slate-200 flex flex-col items-center justify-center py-20 text-center"
        >
        ...
        <div v-else>
          ...
          <div class="bg-white rounded-xl border border-slate-200 p-5 mb-4">
            <div class="flex items-center gap-4">
              <div
                class="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center shrink-0 overflow-hidden"
              >
                <img
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-slate-400 m-0">
                  ...
                  {{ mascotaSeleccionada.breed ?? 'Sin raza' }} · Dueño:
                  {{ mascotaSeleccionada.owner?.name ?? '—' }}
                </p>
              </div>
              <span class="text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded-md"
                >{{ historial.length }} expedientes</span
              >
            </div>
          </div>
          ...
          <div
            ...
            class="bg-white rounded-xl border border-slate-200 flex items-center justify-center py-12"
          >
            <div
          ...
          <div
            ...
            class="bg-white rounded-xl border border-slate-200 flex flex-col items-center justify-center py-14 text-center"
          >
              ...
              <div class="flex items-start justify-between mb-4">
                <div>
                  <p class="text-sm font-semibold text-slate-800 m-0">
                    {{ expediente.appointment?.service?.name ?? 'Consulta' }}
                  </p>
                  <p class="text-xs text-slate-400 mt-0.5 m-0">
                    {{ expediente.created_at?.slice(0, 10) }} · Dr.
                    {{ expediente.veterinarian?.name ?? '—' }}
                  </p>
                </div>
                <span
                  class="text-xs bg-green-50 text-green-700 border border-green-200 px-2 py-1 rounded-md"
                  >Completada</span
                >
              </div>
