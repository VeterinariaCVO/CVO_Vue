...
async function cargarSlots() {
  ...
  const { data: wdData, execute: wdEx } = ApiUseFetch('/working-days').get().json()
  await wdEx()
  const workingDays = wdData.value?.data ?? []
  const wd = workingDays.find((d: any) => d.date?.slice(0, 10) === fechaSeleccionada.value)
  ...
  const { data, execute } = ApiUseFetch(`/time-slots?working_day_id=${wd.id}&status=available`)
    .get()
    .json()
  await execute()
...
async function ejecutarReagendar() {
  ...
  const cita = citaReagendar.value
  const { data, statusCode, execute } = ApiUseFetch(`/cliente/appointments/${cita.id}`)
    .put({ time_slot_id: slotId.value, status: cita.status })
    .json()
  await execute()
    ...
    <Transition name="modal">
      ...
      >
        <div class="bg-white rounded-xl border border-slate-200 p-6 max-w-sm w-11/12">
          ...
          <!-- Error -->
          <div v-if="errorReagendar" class="text-xs text-red-500 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-3">
            {{ errorReagendar }}
          </div>
          ...
          <div class="flex gap-2">
            ...
            >{{ enviandoReagendar ? 'Guardando...' : 'Confirmar' }}</button>
          </div>
        </div>
      </div>
    </Transition>
