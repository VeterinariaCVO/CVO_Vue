...
const emEspecie = ref('Perro') // Por defecto
...
// FIX: Rutas actualizadas para el ROL DE RECEPCIONISTA
async function cargarClientes() {
  try {
    // Antes era /admin/users, ahora es la correcta para el empleado:
    const { data, execute } = ApiUseFetch('/empleado/clients').get().json()
    await execute()

    // Si tu controlador ya trae solo clientes (role 3), los asignamos directo
...
async function cargarMascotas(ownerId: number) {
  ...
  mascotas.value = []
  // Antes era /admin1/pets, ahora usa la de recepcionista:
  const { data, execute } = ApiUseFetch(`/recep/pets?owner_id=${ownerId}`).get().json()
  await execute()
...
async function registrar() {
  ...
  // Antes era /walk-in, ahora usa la ruta que tienes en api.php para el Rol 2:
  const { data, execute } = ApiUseFetch('/recep/walk-in').post(payload).json()
  await execute()
  ...
  const mascotaNombre = modoEmergencia.value ? emNombreMascota.value : (mascotas.value.find(m => m.id === mascotaId.value)?.name ?? 'Paciente')
  ...
  <div class="walkin-root">
    ...
    <div class="walkin-header">
      <div class="walkin-header-inner">
        ...
        <div>
          <h1 class="walkin-title">Walk-in Exprés</h1>
          <p class="walkin-subtitle">Atención inmediata a sala de espera</p>
        </div>
      </div>
    </div>
    ...
    <div class="walkin-body">
      <div class="walkin-card">
        ...
        <div class="walkin-col">
          <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex bg-slate-100 p-1 rounded-xl">
              <button @click="modoEmergencia = false; resetForm()" :class="['px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all', !modoEmergencia ? 'bg-white text-[#1d6bbf] shadow-sm' : 'text-slate-400']">
                Registrado
              </button>
              <button @click="modoEmergencia = true; resetForm()" :class="['px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all flex gap-1 items-center', modoEmergencia ? 'bg-red-500 text-white shadow-sm' : 'text-slate-400']">
          ...
          <div v-if="!modoEmergencia" class="space-y-4">
            <div class="walkin-field">
              ...
              <div class="walkin-input-wrap">
                ...
                <input v-model="busquedaCliente" @input="filtrarClientes" @focus="filtrarClientes" :disabled="!!clienteId" type="text" placeholder="Buscar cliente..." class="walkin-input" />
              </div>

              <div v-if="mostrarDropdown && clientesFiltrados.length > 0 && !clienteId" class="walkin-dropdown">
                <button v-for="cliente in clientesFiltrados.slice(0, 6)" :key="cliente.id" @click="seleccionarCliente(cliente)" class="walkin-dropdown-item">
            ...
            <div class="walkin-field">
              ...
              <p v-else-if="!clienteId" class="walkin-hint">Selecciona un cliente</p>
              <p v-else-if="mascotas.length === 0" class="walkin-hint">Sin mascotas.</p>
              <div v-else class="walkin-pet-grid">
                <button v-for="mascota in mascotas" :key="mascota.id" @click="mascotaId = mascota.id" :class="['walkin-pet-card', mascotaId === mascota.id ? 'walkin-pet-card--active' : '']">
        ...
        <div class="walkin-col">
          ...
          <div class="walkin-field walkin-field--grow mt-2">
          ...
          <div class="walkin-actions">
            ...
            <button @click="registrar" :disabled="enviando" :class="modoEmergencia ? 'bg-red-500 hover:bg-red-600 flex-2 text-white font-bold py-3 rounded-lg flex justify-center items-center gap-2' : 'walkin-btn-primary'">
